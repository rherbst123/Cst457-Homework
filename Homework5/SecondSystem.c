    ...
    while (dentry != NULL) {
        ...
        if (dentry->d_type == DT_DIR) {
            if ((strcmp(dentry->d_name, ".") != 0) && (strcmp(dentry->d_name, "..") != 0)) {
                ...
                pthread_create(&threads[thread_count], NULL, processDirectory, strdup(buffer));
                thread_count++;
            }
        } else if (dentry->d_type == DT_REG) {
            // create a thread for each file
            pthread_create(&threads[thread_count], NULL, processFile, strdup(buffer));
            thread_count++;
        }
...
int main() {
    ...
    FILE *outputFile = freopen("System2.txt" , "a" ,stdout);
    ...
    for (int run = 0; run < num_runs; ++run) {
        ...
        // create the first thread for the main directory
